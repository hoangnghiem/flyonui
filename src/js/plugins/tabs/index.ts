...
class HSTabs extends HSBasePlugin<{}> implements ITabs {
  ...
  private init() {
    ...
    this.toggles.forEach(el => {
      ...
      el.addEventListener(this.eventType, this.onToggleClickListener.find(toggle => toggle.el === el).fn)
    })
  ...
  public destroy() {
    // Clear listeners
    this.toggles.forEach(el => {
      el.removeEventListener(this.eventType, this.onToggleClickListener.find(toggle => toggle.el === el).fn)
    })
    this.onToggleClickListener = []
    if (this.extraToggle) this.extraToggle.removeEventListener('change', this.onExtraToggleChangeListener)
